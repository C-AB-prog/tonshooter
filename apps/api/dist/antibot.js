...
export async function logAction(userId, type, meta) {
 ...
 */
export async function antibotCheckAndMaybeFlag(userId, action, now) {
    ...
    if (!u)
        return { blocked: false, suspicion: 0 };
    if (u.isBotBlocked)
        return { blocked: true, suspicion: u.suspicionScore };
