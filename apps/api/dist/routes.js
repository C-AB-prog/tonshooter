...
import { coinsForHit, energyCost, energyCostWithBoost, canUpgrade, upgradePrice, COINS_PER_CRYSTAL, CRYSTALS_PER_TON, ENERGY_MAX, BOOST_TON_COST, BOOST_COOLDOWN_MS, UPGRADE_TO_LEVEL5_TON_COST, LEVEL5_TON_COST, REFERRAL_REWARD_COINS, REFERRAL_ACTIVE_SHOTS, REFERRAL_ACTIVE_HITS, REFERRAL_ACTIVE_WINDOW_HOURS, } from "./economy.js";
...
router.post("/admin/grant", requireAuth, async (req, res) => {
    ...
    if (body.coins !== undefined)
        data.coins = { increment: BigInt(body.coins) };
    if (body.crystals !== undefined)
        data.crystals = { increment: BigInt(body.crystals) };
    if (body.tonBalance !== undefined)
        data.tonBalance = { increment: new Prisma.Decimal(body.tonBalance) };
    if (body.energy !== undefined) {
        data.energy = body.energy;
        data.energyUpdatedAt = new Date();
    }
    if (body.weaponLevel !== undefined)
        data.weaponLevel = body.weaponLevel;
    if (body.rangeLevel !== undefined)
        data.rangeLevel = body.rangeLevel;
    if (body.resetBoost) {
        data.boostActiveUntil = null;
        data.boostCooldownUntil = null;
    }
    if (body.resetWithdrawal) {
        data.lastWithdrawalAt = null;
    }
    if (body.resetAntibot) {
        data.isBotBlocked = false;
...
async function applyTonPurchase(uid, kind) {
    const now = new Date();
    if (kind === "boost") {
...
router.post("/ton/purchase/intent", requireAuth, async (req, res) => {
    ...
    if (kind === "boost") {
        ...
        if (!u)
            return res.status(404).json({ error: "user_not_found" });
        if (u.weaponLevel !== 4)
            return res.status(409).json({ error: "need_level_4" });
    }
...
router.post("/ton/purchase/confirm", requireAuth, async (req, res) => {
    ...
    if (!purchase || purchase.userId !== uid)
    ...
    const data = await toncenterFetch("/transactions", {
    ...
    const matched = txs.find((t) => {
        ...
        if (!inMsg)
            return false;
        if (String(inMsg?.value ?? "") !== expectedValue)
            return false;
        const comment = inMsg?.message_content?.decoded?.comment;
        if (String(comment ?? "") !== expectedComment)
            return false;
        const src = inMsg?.source;
        if (expectedSender && String(src ?? "") !== expectedSender)
            return false;
        // Must be ordinary transaction (best-effort)
        if (t?.description?.type && String(t.description.type) !== "ord")
            return false;
        return true;
    });
    if (!matched) {
    ...
    if (!kind)
        return res.status(500).json({ error: "unknown_purchase_item" });
...
router.post("/ton/purchase/mock", requireAuth, async (req, res) => {
    if (!config.allowMockTon)
        return res.status(403).json({ error: "mock_disabled" });
    const { uid } = req.auth;
    const now = new Date();
    const schema = z.object({
    ...
    if (purchase === "upgrade_weapon_5") {
        if (user.weaponLevel !== 4)
            return res.status(409).json({ error: "invalid_level", message: "weapon must be level 4" });
        const check = canUpgrade(user.weaponLevel + 1, user.rangeLevel, "weapon");
        if (!check.ok)
            return res.status(409).json({ error: "upgrade_blocked", reason: check.reason });
    ...
    if (purchase === "upgrade_range_5") {
        if (user.rangeLevel !== 4)
            return res.status(409).json({ error: "invalid_level", message: "range must be level 4" });
...
router.post("/shot/start", requireAuth, async (req, res) => {
    ...
    if (ab.blocked)
        return res.status(403).json({ error: "bot_suspected" });
    const user = await prisma.user.findUnique({ where: { id: uid } });
    if (!user)
        return res.status(404).json({ error: "not_found" });
    const regen = applyEnergyRegen(user.energy, user.energyUpdatedAt, now);
...
router.post("/shot/fire", requireAuth, async (req, res) => {
    ...
    if (session.used)
        return res.status(409).json({ error: "session_used" });
...
router.post("/upgrade", requireAuth, async (req, res) => {
    ...
    if (!can.ok)
        return res.status(409).json({ error: "upgrade_blocked", reason: can.reason });
    ...
    if (nextLevel === 5) {
        ...
        if (which === "weapon")
            data.weaponLevel = { increment: 1 };
        if (which === "range")
            data.rangeLevel = { increment: 1 };
    ...
    if (!price)
        return res.status(409).json({ error: "upgrade_unavailable" });
    if (user.coins < BigInt(price))
        return res.status(409).json({ error: "not_enough_coins" });
    const data = { coins: { decrement: BigInt(price) } };
    if (which === "weapon")
        data.weaponLevel = { increment: 1 };
    if (which === "range")
        data.rangeLevel = { increment: 1 };
...
router.post("/exchange", requireAuth, async (req, res) => {
    ...
    if (user.crystals < crystalsNeed)
        return res.status(409).json({ error: "not_enough_crystals" });
    const tonAdd = amount;
    const updated = await prisma.user.update({
        where: { id: uid },
        data: {
...
router.post("/boost/buy", requireAuth, async (req, res) => {
    ...
    const method = parsed?.method ?? "ton";
    const user = await prisma.user.findUnique({ where: { id: uid } });
    if (!user)
        return res.status(404).json({ error: "not_found" });
    const cooldownUntil = user.boostCooldownUntil?.getTime() ?? 0;
...
router.post("/tasks/claim", requireAuth, (req, res, next) => {
    (async () => {
        ...
        if (!opened.openToken || opened.openToken !== openToken)
            return res.status(409).json({ error: "need_open_first" });
        ...
        try {
            const updatedUser = await prisma.$transaction(async (tx) => {
                ...
                if (!o || !o.openToken || o.openToken !== openToken)
                    throw new Error("need_open_first");
                if (o.openTokenExpiresAt && o.openTokenExpiresAt.getTime() < Date.now())
                    throw new Error("need_open_first");
                ...
                const u = await tx.user.update({
                    where: { id: uid },
                    data: t.rewardType === RewardType.COINS
...
router.post("/withdraw", requireAuth, async (req, res) => {
    ...
    if (ab.blocked)
        return res.status(403).json({ error: "bot_suspected" });
    if (amountTon < 1)
        return res.status(409).json({ error: "min_withdraw_1_ton" });
    if (amountTon > 25)
        return res.status(409).json({ error: "max_withdraw_25_ton" });
    const user = await prisma.user.findUnique({ where: { id: uid } });
    if (!user)
        return res.status(404).json({ error: "not_found" });
...
async function checkChatMember(chatId, userId, botToken) {
