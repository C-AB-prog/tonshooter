...
export function energyCost(weaponLevel, rangeLevel) {
    return 1 + weaponLevel + rangeLevel;
}
export function energyCostWithBoost(baseCost, boost) {
...
export function coinsForHit(weaponLevel, rangeLevel) {
...
export function coinsToCrystals(coins) {
    return coins / BigInt(COINS_PER_CRYSTAL);
}
export function crystalsToTon(crystals) {
...
export function upgradePrice(currentLevel) {
    if (currentLevel >= MAX_LEVEL)
        return null;
    return UPGRADE_PRICES[currentLevel] ?? null;
}
export function canUpgrade(weaponLevel, rangeLevel, which) {
    const w = weaponLevel;
    const r = rangeLevel;
    const nextW = which === "weapon" ? w + 1 : w;
    const nextR = which === "range" ? r + 1 : r;
    if (nextW > MAX_LEVEL || nextR > MAX_LEVEL)
        return { ok: false, reason: "Достигнут максимальный уровень" };
    // ТЗ: |WeaponLevel - RangeLevel| ≤ 3
    if (Math.abs(nextW - nextR) > 3)
        return { ok: false, reason: "Разница уровней не должна превышать 3" };
    const price = upgradePrice(which === "weapon" ? w : r);
    if (price == null)
        return { ok: false, reason: "Улучшение недоступно" };
    return { ok: true };
}
