...
export function applyEnergyRegen(currentEnergy, energyUpdatedAt, now) {
    if (currentEnergy >= ENERGY_MAX)
        return { energy: ENERGY_MAX, updatedAt: energyUpdatedAt };
    const diffMs = now.getTime() - energyUpdatedAt.getTime();
    if (diffMs <= 0)
        return { energy: currentEnergy, updatedAt: energyUpdatedAt };
